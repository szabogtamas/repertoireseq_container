...
```{r}
bg_idx <- sample(nrow(real_bcr_seqs), 240)

background_pool <- real_bcr_seqs[bg_idx,]
```

```{r}
ag1_idx <- sample(nrow(real_bcr_seqs[-bg_idx,]), 20)
ag1_pool <- real_bcr_seqs[-bg_idx,][ag1_idx,]
```

```{r}
ag2_idx <- sample(nrow(real_bcr_seqs[-bg_idx,][-ag1_idx,]), 20)
ag2_pool <- real_bcr_seqs[-bg_idx,][-ag1_idx,][ag2_idx,]
```

```{r}
common_ag_idx <- sample(nrow(real_bcr_seqs[-bg_idx,][-ag1_idx,][-ag2_idx,]), 4)
...
for (i in seq(1, 4)){
  ...
  sim_sample_table <- background_pool %>%
    sample_n(80) %>%
    mutate(nClone = sample(25:50, n(), replace=T))
...
for (i in seq(5, 8)){
  ...
  major_clone_table <- ag1_pool %>%
    sample_n(15) %>%
    mutate(nClone = sample(200:300, n(), replace=T))
  
  common_ag_tab <- common_ag_pool %>%
    mutate(nClone = sample(80:120, n(), replace=T))
